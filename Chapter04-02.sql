...
@GROUPID AS VARCHAR(10), 
@PRVGROUPID AS VARCHAR(10), 
@STRING AS VARCHAR(10), @AGGSTRING VARCHAR(8000);
...
SELECT GROUPID, [1] + COALESCE (',' + [2], '') 
	...
	FROM 
	(SELECT GROUPID, STRING, ROW_NUMBER() OVER(PARTITION BY GROUPID ORDER BY MEMBERID) AS RN
...
ORDER BY memberid
FOR XML PATH(''), TYPE).value('.[1]', 'VARCHAR(MAX)'), 1, 1, '') AS
string
