...
SELECT 
GROUPING_ID(CUSTID, EMPID, YEAR(ORDERDATE), MONTH(ORDERDATE), DAY(ORDERDATE)
), 
CUSTID, 
EMPID, 
	YEAR(ORDERDATE) AS YEAR, 
...
USE tempdb;
IF OBJECT_ID(N'dbo.MyGroupingSets', N'U') IS NOT NULL DROP TABLE
dbo.MyGroupingSets;
GO
SELECT
GROUPING_ID(
...
ON dbo.MyGroupingSets(grp_id, custid, empid, orderyear, ordermonth,
orderday);
